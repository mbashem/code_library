...
public:
	...
	Hash(int mxSize, ll _base, ll _mod) // Max size
	{
		this->base = _base;
		this->mod = _mod;
		...
		{
			base_pow[i] = (int)(base_pow[i - 1] * base % mod);
		}
	}
	...
	void init(string s)
	{
		...
		for (int i = 1; i <= n; i++)
		{
			f_hash[i] = (int)((f_hash[i - 1] * base + int(s[i - 1])) % mod);
		}

		for (int i = n; i >= 1; i--)
		{
			r_hash[i] = (int)((r_hash[i + 1] * base + int(s[i - 1])) % mod);
		}
	}

	int forward_hash(int l, int r)
	{
		int h = (int)(f_hash[r + 1] - (1LL * base_pow[r - l + 1] * f_hash[l]) % mod);
		return h < 0 ? (int)mod + h : h;
	}

	int reverse_hash(int l, int r)
	{
		int h = (int)(r_hash[l + 1] - (1LL * base_pow[r - l + 1] * r_hash[r + 2]) % mod);
		return h < 0 ? (int)mod + h : h;
	}
};
